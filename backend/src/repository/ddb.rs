...
impl DDBRepository {
    ...
    pub async fn put_post(&self, post: Post) -> Result<(), DDBError> {
        let mut request = self
            ...
            .item("pK", AttributeValue::S(post.blog_id))
            .item("sK", AttributeValue::S(post.post_id))
    ...
    pub async fn put_blog(&self, blog: NewBlog) -> Result<(), DDBError> {
        let request = self
            ...
            .put_item()
            .table_name(&self.table_name)
            .item("pK", AttributeValue::S(blog.blog_id))
            .item("sK", AttributeValue::S(String::from("")))
            .item("title", AttributeValue::S(blog.title.clone()))
            .item("about", AttributeValue::S(blog.about.clone()));
    ...
    pub async fn get_blog(
        ...
        if let Some(oldest) = oldest {
            res = res
                .expression_attribute_values(":oldest", AttributeValue::S(oldest))
                .key_condition_expression("#post_id > :oldest")
        }

        if let Some(newest) = newest {
            res = res
                .expression_attribute_values(":newest", AttributeValue::S(newest))
